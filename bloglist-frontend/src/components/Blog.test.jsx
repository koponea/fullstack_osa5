...
describe('<Blog />', () => {
  ...
  describe('tests with mock reactions', () => {
    ...
    test('like button reacts', async () => {
      ...
      expect(await screen.findByTestId('like-button').textContent).toContain(regex)
      expect(await screen.findByTestId('like-button')).toBeVisible()

      expect(mockHandler.mock.calls).toHaveLength(2)

      screen.debug((await screen.findByTestId('like-button')).parentElement)
    })
  })
})
